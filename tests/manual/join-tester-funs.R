...
join.sql.equal = function(x, i, nomatch=NA_integer_, mult="all", which=TRUE, allow.cartesian=TRUE, on, verbose=FALSE, .conn, .drop=TRUE) {
    ...
    sql.on = paste(sapply(seq_along(o.on), function(i) {
        r = paste(paste0("x.", x.on[i]), o.on[i], paste0("i.",i.on[i]))
        if (o.on[i] == "=") { # match NULL=NULL as in R
            sprintf("(%s OR (%s IS NULL AND %s IS NULL))", r, paste0("x.", x.on[i]), paste0("i.", x.on[i]))
        } else r
    }), collapse = " AND ")
    ...
    if (mult == "all") {
        ...
        mult_join = sprintf("SELECT mult_all.%s FROM mult_all INNER JOIN mult_one ON (mult_all.\"x.row_id\"==mult_one.\"x.row_id\" OR (mult_all.\"x.row_id\" IS NULL AND mult_one.\"x.row_id\" IS NULL)) AND (mult_all.\"i.row_id\"==mult_one.\"i.row_id\" OR (mult_all.\"i.row_id\" IS NULL AND mult_one.\"i.row_id\" IS NULL))",
...
batch.join.sql.equal = function(x, i, on.v, nomatch.v = c(NA_integer_, 0L), mult.v = c("all","last","first"), which.v = c(TRUE, FALSE), verbose=TRUE) {
    ...
    if (len > (len.warn <- getOption("tests.length.warning", 1e3))) warning(sprintf("You are about to run %s number of tests. To suppress this warning use 'tests.length.warning' option, set to numeric threshold or Inf.", len), immediate.=TRUE)
    r = list()
    if (isTRUE(verbose)) {
        pt = proc.time()[[3L]]
...
populate.on.v = function(x, i, x.on, i.on, cap=1e3, seed=123, ops=c(">=", "<=", ">", "<", "==")) {
    stopifnot(
    ...
    for (i in seq_along(x.on)) {
        r[[x.on[i]]] = if (is.character(x[[x.on[i]]]) | is.factor(x[[x.on[i]]])) "==" else ops
    }
