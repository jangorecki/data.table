...
- [x] maybe people uses times=100 because they have small data, and it goes fast for them, so they want the average, because if something is very fast then it is likely to be subject to noise. but they miss the point that it is still fast enough to not really care about performance  
- [x] Yes they all miss that point.  
- [x] First section: "scale matters". Explain that if we view the dataset as a function of n (number of rows), then it would be impossible to find any code whose time is proportional to n. Which is why when measuring performance the most important thing is to measure it at different ns. It is a mortal sin to microbenchmark some code for say n=10, and then assume that any inferences you draw will hold for n=10,000,000.  
- [x] Second section: "going wide". Another dimension for a data frame is the number of columns. Some operations take the same amount of time no matter how wide the data frame is, while others are not that lucky. This may be especially true when comparing against implementations that like to create many copies of the data, or if they store the data row-wise, etc.  
- [x] "miscellaneous". Other things, like running in --vanilla mode, making sure you have a production version of the package and not a debug one, etc. Also about being polite, and if you're about to publish some benchmarking results against another library -- reach out to the authors of that other package to check with them if you're using their library correctly.  
- [x] Third section: "disk cache". This is about reading a file the first time vs. the second time (and how in most use cases it is the first time that only matters).  
- [x] "internal optimizations". Some implementations may cache / memoize certain results, which makes subsequent runs faster compared to the first. Always try to time the code when running once, and then compare with multiple runs from the microbenchmark. Are the timings approximately same? If there's a big discrepancy, you might need to look into the implementation.  
- [x] what is call overhead   
- [ ] one simple demonstration of why microbenchmark(times=100) is stupid.  
- [ ] A reproducible demo in the vignette.   
- [ ] We have to demonstrate that scaling up data size can (often) arrive at a different conclusion about relative speed.  We actually have to demo that in a reproducible example before their minds will grasp it.  
- [ ] Perhaps the fact that 3 calls to getOption() appear in the argument list of [.data.table could be used as an example. The default could be added back to getOption() and the microbenchmark results presented. Some inferences then made about the operation (say, adding a column like above). Then continue with an Rprof() and discover, "oh, wait, all the time is in getOption? I am not timing the operation I thought I was!". Then explain call overhead and demonstrate what we mean when we say that the inference made earlier is correct or not depending on whether the user does need to call DT[...] in a loop or not. In that case, actually scale up the data so that the call overhead in DT[...] becomes an insignificant percentage, and demonstrate using a reproducible example the inference then swapping around. It could be said that this is a disadvantage of data.table in that users need to know this and use a different function (set*) to avoid this call overhead.  
...
You should not forget about taking extra care about environment in which you are runnning benchmark. It should be striped out from startup configurations, so consider `R --vanilla` mode. Any extra configurations should be well documented in benchmark. Be sure to use recent releases of tools you are benchmarking.
You should not forget about being polite, and if you're about to publish some benchmarking results against another library -- reach out to the authors of that other package to check with them if you're using their library correctly.
